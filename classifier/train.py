...
def create_datasets(cfg: DictConfig, train_processor, val_processor):
    ...
    if cfg.dataset.max_train_samples is not None:
        train_dataset.dataset = train_dataset.dataset.select(range(cfg.dataset.max_train_samples))
    
    if cfg.dataset.max_val_samples is not None:
        val_dataset.dataset = val_dataset.dataset.select(range(cfg.dataset.max_val_samples))
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        if cfg.trainer.grad_clip.enabled:
            torch.nn.utils.clip_grad_norm_(
                model.parameters(),
                cfg.trainer.grad_clip.max_norm
            )
...
def main(cfg: DictConfig):
    ...
    if cfg.trainer.save_last:
        save_checkpoint(
            model, optimizer, cfg.trainer.epochs, best_acc,
